...
A fast, SEO-optimized marketing website for **Amguard**, a New York City **sidewalk shed
specialist**. The site markets sidewalk sheds (sidewalk bridging) as the primary service —
coordination, installation, rental management, maintenance and removal — with scaffolding,
construction fencing and debris chutes positioned as expanding services. Built as a
lightweight static site (plain HTML/CSS/JS) for maximum Core Web Vitals performance and
clean SEO — ideal for Google Ads landing pages.

## ✨ What's included

- **9 pages** — Home (sidewalk-shed focused), Services overview, the Sidewalk Sheds landing
  page, a Scaffolding & Access page (framed as an expanding service), Gallery, About, FAQ,
  Contact — plus a Thank-You page and a custom 404
- **Working lead capture** — quote/contact forms wired to **Netlify Forms** (no backend
  needed). The sidewalk-shed form pre-tags submissions with the service.
...
- **Ads-ready** — a focused sidewalk-shed landing page (better Google Ads Quality Score than
  a generic homepage), plus a ready-to-fill Google Ads / GA4 conversion hook on the
  Thank-You page
...
```
.
├── index.html            # Home — sidewalk sheds primary, "Who We Serve" + Expanding Services
├── services.html         # Services overview (sidewalk sheds + lifecycle + expanding services)
├── sidewalk-sheds.html   # Primary service landing page (+ FAQ schema)
├── scaffolding.html      # Scaffolding & Temporary Access — expanding service (+ FAQ schema)
...
| `info@amguard.com` | Your real email | all `.html` |
| `https://www.amguard.com` | Your real domain | all `.html`, `sitemap.xml`, `robots.txt` |
...
- **Point your ad groups at the matching landing page** for the best Quality Score:
  - "sidewalk shed" / "sidewalk bridging" / "Local Law 11 shed" ads → `/sidewalk-sheds.html` (your primary money page)
  - "scaffolding" / "temporary access" ads → `/scaffolding.html` (expanding service)
  - `/bridging` and `/bridging.html` 301-redirect to the sidewalk shed page.
...
© Amguard. Built for scaffolding, sidewalk sheds & bridging in New York City.
